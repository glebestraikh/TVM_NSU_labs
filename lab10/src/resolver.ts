...
function collectPredicateVariables(pred: Predicate, names: Set<string>) {
    if (!pred) return;
    ...
    if (p.kind === 'comparison') {
        collectExprVariables(p.left, names);
        collectExprVariables(p.right, names);
        return;
    }

    if (p.kind === 'not') {
        collectPredicateVariables(p.condition, names);
        return;
    }

    if (p.kind === 'and' || p.kind === 'or' || p.kind === 'implies') {
        collectPredicateVariables(p.left, names);
        collectPredicateVariables(p.right, names);
        return;
    }

    if (p.kind === 'paren') {
        collectPredicateVariables(p.inner, names);
        return;
    }

    if (p.kind === 'forall' || p.kind === 'exists') {
        // Вложенный квантор - рекурсивно обрабатываем
        collectPredicateVariables(p.predicate, names);
        return;
    }

    if (p.kind === 'formulaRef') {
        // Формула ссылается на переменные через аргументы
        if (p.args && Array.isArray(p.args)) {
            for (const arg of p.args) {
                collectExprVariables(arg, names);
            }
        }
        return;
    }
...
function collectExprVariables(expr: any, names: Set<string>) {
    if (!expr) return;
    ...
    switch (expr.type) {
        ...
        case 'const':
            break;
        case 'binop':
            collectExprVariables(expr.left, names);
            collectExprVariables(expr.right, names);
            break;
        case 'unary':
            collectExprVariables(expr.argument, names);
            break;
        case 'funccall':
            if (expr.args) {
                expr.args.forEach((arg: any) => collectExprVariables(arg, names));
            }
            break;
        case 'arraccess':
            names.add(expr.name);
            collectExprVariables(expr.index, names);
            break;
    }
}
